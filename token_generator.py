...
def get_access_token(username, password):
    ...
    KEYCLOAK_ACCESS_TOKEN_URL = KEYCLOAK_BASE_URI + '/realms/' + KEYCLOAK_REALM_NAME + '/protocol/openid-connect/token'

    http = urllib3.PoolManager(ca_certs=certifi.where())
    data = {"client_id": KEYCLOAK_CLIENT_ID, "client_secret": KEYCLOAK_CLIENT_SECRET, "grant_type": "password",
            "username": username, "password": password}
    encoded_data = urllib.parse.urlencode(data)

    headers = {'Content-Type': 'application/x-www-form-urlencoded'}
    try:
        request = http.request('POST', KEYCLOAK_ACCESS_TOKEN_URL, headers=headers, body=encoded_data)
        response_data = json.loads(request.data.decode('utf-8'))
        response = {
            'statusCode': 200,
            'body': response_data['access_token']
        }
